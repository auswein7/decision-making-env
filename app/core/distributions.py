...
class Distribution:
    ...
    #             k=1
    #         )[0]
    #         return set(selected_action), probabilities
    #     else:
    #         return random.choice([a for a in action_scores]), None
    ...
    def approximate_best_response(self, action_scores):
        ...
        max_score = max(action_scores.values())
        threshold = self.beta * max_score

        # Keep all actions with score ≥ beta · max_score
        candidates = [a for a, score in action_scores.items() if score >= threshold]

        if not candidates:
            candidates = list(action_scores.keys())

        selected = random.choice(candidates)
        probabilities = {a: 1 / len(candidates) for a in candidates}

        return set(selected), probabilities
